...
describe('postUpidForMachine', () => {
  it('posts an unsorted imported document in orderIndex execution order without mutating it', () => {
    ...
    expect(posted.operations.map((operation) => operation.operationId)).toEqual([
      first.id,
      second.id
    ]);
