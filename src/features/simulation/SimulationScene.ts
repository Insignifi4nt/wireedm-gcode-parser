...
export async function createSimulationScene(
  ...
  try {
    ...
    scene.background = new THREE.Color('#111b26');
    ...
    const ambient = new THREE.HemisphereLight('#c1ddff', '#394759', 2.5);
    ambient.up.set(0, 0, 1);
    scene.add(ambient);
    const key = new THREE.DirectionalLight('#fff5e0', 4);
    ...
    const rim = new THREE.DirectionalLight('#6ccbf5', 2.4);
    rim.position.copy(center).add(new THREE.Vector3(-size, size, size));
    rim.target.position.copy(center);
    scene.add(rim, rim.target);
    const floorZ = plan.settings.supportFloorZ ?? stock.bottomZ - size;
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(size * 30, size * 30),
      new THREE.MeshStandardMaterial({ color: '#172431', roughness: 0.85, metalness: 0.15 }));
    floor.position.set(center.x, center.y, floorZ - 0.03);
    floor.receiveShadow = true;
    scene.add(floor);
    const grid = new THREE.GridHelper(size * 6, 60, '#526979', '#293b49');
    grid.rotation.x = Math.PI / 2;
    grid.position.set(center.x, center.y, floorZ);
    scene.add(grid);

    const stockMaterial = new THREE.MeshStandardMaterial({ color: '#94a8b6', roughness: 0.35, metalness: 0.7 });
    ...
    function extrude(polygon: readonly Point2[], holes: readonly (readonly Point2[])[]) {
      return new THREE.ExtrudeGeometry(polygonShape(polygon, holes, center), { depth: stock.thickness, bevelEnabled: false, curveSegments: 1 });
    }
    ...
    body.position.set(center.x, center.y, stock.bottomZ); body.castShadow = true;
    scene.add(body);
    ...
    const wireRadius = Math.max(plan.settings.wireDiameter / 2, size * 0.0007);
    ...
    const guideHeight = size * 0.075;
    const guideGeometry = new THREE.CylinderGeometry(plan.settings.guideRadiusMm, plan.settings.guideRadiusMm * 1.5, guideHeight, 24);
    ...
    function render() { if (!disposed && host.clientWidth && host.clientHeight) renderer.render(scene, camera); }
    function resize() {
      if (disposed) return;
    ...
    const guideRadius = Math.max(plan.settings.guideRadiusMm * 1.5, wireRadius);
    ...
    function fitBounds(bounds: THREE.Box3, direction: THREE.Vector3) {
      ...
      for (const x of [bounds.min.x, bounds.max.x]) for (const y of [bounds.min.y, bounds.max.y]) for (const z of [bounds.min.z, bounds.max.z]) {
        ...
        nearestOffset = Math.max(nearestOffset, depth);
      ...
      camera.near = Math.max(0.001, Math.min(radius / 10_000, (distance - nearestOffset) / 2));
      camera.far = Math.max(size * 500, distance + radius * 4);
    ...
    function setView(view: 'isometric' | 'top' | 'front') {
      ...
      fitBounds(simulationBounds, direction);
    }
    resize(); setView('isometric');
    return {
      ...
      update(snapshot) {
        ...
        const visiblePieces = new Set(snapshot.pieces.map(piece => piece.id));
        for (const [id, item] of pieces) item.mesh.visible = visiblePieces.has(id);
        for (const piece of snapshot.pieces) {
          let item = pieces.get(piece.id);
          if (!item) {
            const mesh = new THREE.Mesh(extrude(piece.polygon, piece.holes), stockMaterial.clone());
            mesh.material.color.set('#b5c7cd'); mesh.castShadow = true;
        ...
        activeTrail.visible = active.vertexCount > 0;
      ...
      setMachineVisible(visible) {
        if (disposed) return;
        machineGroup.visible = visible; render();
      },
      fitMachine() {
        if (disposed || !lastModel) return;
