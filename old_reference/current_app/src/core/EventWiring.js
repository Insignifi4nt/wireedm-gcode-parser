...
import { createDisplayedLineMapping, summarizeParseResult } from '../utils/GCodeStats.js';
...
export function attachEventWiring(app) {
      ...
      if (app.gcodeDrawer && app.toolbar?.fileHandler?.loadedData) {
        ...
        try {
          ...
          app.gcodeDrawer.setContent({
            text: stripped,
            mapping: createDisplayedLineMapping(stripped, data.path, app.parser)
          });
        } catch (_e) {
          try {
            ...
            app.gcodeDrawer.setContent({
              text: canonical,
              mapping: createDisplayedLineMapping(canonical, data.path, app.parser)
            });
          } catch (_e2) {
            app.gcodeDrawer.setContent({
              text: raw,
              mapping: createDisplayedLineMapping(raw, data.path, app.parser)
            });
          }
        }
      }
  ...
  if (app.gcodeDrawer) {
    ...
    on('drawer:selection:changed', ({ indices }) => {
      app.canvas?.setSelectionHighlights(indices);
    });
    on('drawer:pins:changed', ({ indices }) => {
      app.canvas?.setPinnedHighlights(indices);
    });
    ...
    on('drawer:content:changed', async ({ text }) => {
      try {
        ...
        app.gcodeDrawer.setContent({
          text: normalizedText,
          mapping: createDisplayedLineMapping(normalizedText, result.path, app.parser),
