 ...
 *   - selectionHighlights: Set<number>
 *   - pinnedHighlights: Set<number>
...
export function renderPath(ctx, viewport, gcodePath, opts = {}) {
  ...
  const selectionHighlights = opts.selectionHighlights || new Set();
  const pinnedHighlights = opts.pinnedHighlights || opts.persistentHighlights || new Set();
  const markerRenderer = typeof opts.markerRenderer === 'function' ? opts.markerRenderer : null;
  const drawSegments = opts.drawSegments !== false;
  const drawHighlights = opts.drawHighlights !== false;

  for (let index = 0; index < gcodePath.length; index++) {
    const move = gcodePath[index];

    if (drawSegments && index > 0) {
    ...
    // Highlight hovered/selected/pinned endpoints
    const hovered = hoverHighlight && hoverHighlight.type === 'point' && hoverHighlight.index === index;
    const selected = selectionHighlights.has(index);
    const pinned = pinnedHighlights.has(index);
    const highlight = hovered || selected || pinned;
    if (drawHighlights && highlight && markerRenderer) {
      ...
      if (hx !== undefined && hy !== undefined) {
        markerRenderer(
          { x: hx, y: hy },
          {
            ...
            COLOR: pinned ? '#ff4d4d' : '#ffa500',
            RADIUS_PX: pinned ? 4 : 3,
            FONT: 'bold 13px Arial',
            LABEL: `${pinned ? 'PIN ' : ''}L${move.line || index}`
          }
        );
      }
    }
