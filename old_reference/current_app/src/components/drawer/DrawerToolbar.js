...
export class DrawerToolbar {
  ...
  _render() {
    ...
    header.innerHTML = `
      ...
      <div class="gcode-drawer-actions">
        <button class="gcode-action-btn gcode-clear-pins-btn" data-action="clear-pins" title="Clear pinned canvas highlights" aria-label="Clear pinned canvas highlights" hidden>📌</button>
    ...
    ctx.innerHTML = `
      <div class="gcode-selection-info">
        <button class="gcode-selection-counter" data-action="clear-selection" type="button" title="Clear selected lines"></button>
      </div>
  ...
  _bindEvents() {
    ...
    on('[data-action="clear-selection"]', 'click', () => this.handlers.onClearSelection?.());
    on('[data-action="clear-pins"]', 'click', () => this.handlers.onClearPins?.());
  ...
  updateSelectionUI(hasSelection, count) {
    ...
    if (selectionCounter && hasSelection) {
      ...
      selectionCounter.setAttribute('aria-label', `Clear ${count} selected line${count !== 1 ? 's' : ''}`);
    }
  ...
  updatePinnedUI(count) {
    const clearPinsBtn = this.container.querySelector('[data-action="clear-pins"]');
    if (!clearPinsBtn) return;

    const hasPins = count > 0;
    clearPinsBtn.hidden = !hasPins;
    clearPinsBtn.title = hasPins
      ? `Clear ${count} pinned canvas highlight${count !== 1 ? 's' : ''}`
      : 'Clear pinned canvas highlights';
  }
