...
export class ArcUtils {
  ...
  static calculateArcParameters(startX, startY, endX, endY, centerX, centerY, clockwise) {
    ...
    const fullCircle = ArcUtils.isFullCircle(startX, startY, endX, endY, radius);
    
    let angleSpan = endAngle - startAngle;

    if (fullCircle) {
      angleSpan = clockwise ? -2 * Math.PI : 2 * Math.PI;
    } else {
  ...
  /**
   * Check whether an arc is a full circle.
   * @param {number} startX - Arc start X coordinate
   * @param {number} startY - Arc start Y coordinate
   * @param {number} endX - Arc end X coordinate
   * @param {number} endY - Arc end Y coordinate
   * @param {number} radius - Arc radius
   * @returns {boolean} True if start/end match and radius is non-zero
   */
  static isFullCircle(startX, startY, endX, endY, radius) {
    const eps = 1e-9;
    return radius > eps &&
      Math.abs(startX - endX) <= eps &&
      Math.abs(startY - endY) <= eps;
  }
  ...
  static calculateArcBounds(startX, startY, endX, endY, centerX, centerY, clockwise) {
    const { radius, startAngle, endAngle, fullCircle } = ArcUtils.calculateArcParameters(
    ...
    if (fullCircle) {
      bounds = BoundsUtils.updateBounds(bounds, centerX - radius, centerY - radius);
      bounds = BoundsUtils.updateBounds(bounds, centerX + radius, centerY + radius);
      return bounds;
    }
