...
function extractXY(raw) {
  ...
  const mX = noComment.match(new RegExp('X\\s*(' + num + ')', 'i'));
  const mY = noComment.match(new RegExp('Y\\s*(' + num + ')', 'i'));
...
function extractIJ(raw) {
  ...
  // Accept integers, leading/trailing decimals, and optional scientific notation
  const num = '[-+]?(?:\\d+(?:\\.\\d*)?|\\.\\d+)(?:[Ee][-+]?\\d+)?';
  const mI = noComment.match(new RegExp('I\\s*(' + num + ')', 'i'));
  const mJ = noComment.match(new RegExp('J\\s*(' + num + ')', 'i'));
...
function getGCommand(raw) {
  if (!raw) return null;
  const cleaned = canonicalizeMotionCodes(dropLeadingBlockNumber(stripInlineComments(raw))).toUpperCase();
  const match = cleaned.match(/^(G[0-3])(?=\D|$)/);
...
function convertG0ToG1(raw) {
  ...
  const replaced = canon.replace(/^(\s*(?:N\d+(?:\s+|(?=[A-Z%])))?)(G0)(?=\D|$)/i, (_, p1) => `${p1}G1`);
...
function updateArcCommand(originalLine, newI, newJ) {
  ...
  // Replace I value
  const iPattern = /I\s*[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[Ee][-+]?\d+)?/i;
  ...
  // Replace J value
  const jPattern = /J\s*[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[Ee][-+]?\d+)?/i;
