...
export function normalizeToISO(inputText, options = {}) {
  ...
  for (let i = 0; i < srcLines.length; i++) {
    ...
    // Drop program delimiter lines, including percent lines with parenthesized labels.
    if (s.startsWith('%')) continue;

    // Strip comments (retain anything outside ';' and parenthesized comments)
    if (stripSemicolon) {
      const idx = s.indexOf(';');
      if (idx >= 0) {
    ...
    // Remove existing block numbers so export always produces a clean sequence.
    s = stripLeadingBlockNumber(s);
...
export function stripForEditing(inputText) {
  ...
  for (; i < lines.length; i++) {
    ...
    if (/^G92(?=\D|$)/i.test(t)) {
      ...
      const hasX = new RegExp(`X\\s*${num}`, 'i').test(t);
      const hasY = new RegExp(`Y\\s*${num}`, 'i').test(t);
      if (!hasX && !hasY) {
...
function stripLeadingBlockNumber(text) {
