...
10. Complete Check post, Build package and Inspect package in the browser. Compatible browsers can use the equivalent WebMCP tools documented at `/documentation/agents/`; read the page context and use its current input version. Inspect golden-output changes against evidence; never update expected text merely to make a test pass. Return the downloaded validation report.
