...
- **Fixed P2: rename dialog initialization race.** The initial focus effect could run while the name was still empty, making the first keyboard wrap skip the disabled Rename action. Populate/reset the dialog before paint. The existing keyboard browser regression exposed this in the final full run. Also align the delete dialog's destination label with Archive.
