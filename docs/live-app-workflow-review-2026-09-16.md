...
Follow-up compensation review: the [Charmilles CT-Millennium manual](https://www.scribd.com/document/459168666/FIX40-cc-SL-program-vH-en-pdf) states that `G40`, `G41`, and `G42` are modal until another of those commands or reset. The [Robofil 290 guide](https://www.scribd.com/document/944739143/manual-charmilles-robofil-290) describes `G38` as the perpendicular offset entry when a normal intersection is unavailable. These are manufacturer materials for adjacent controls, not a verified Robofil 100 programming manual. The repository's historical ten-contour file also retains one side across rapid repositioning. Package 2.4.0 therefore keeps compensation across clear rapid moves and manual rethreading, emits no new offset block for the same side, and emits a direct `G41`/`G42` plus `G38 D0` for a changed side. It emits `G40` and `G39` only at program end. The mixed-side result remains a candidate for controller graphics and supervised dry-run validation.

Contour Setup now exposes Automatic as a selectable compensation choice. It derives kept material from a recognized exterior, island, or hole role and resolves the wire side from travel direction. The detailed saved intent and winding are collapsed by default to keep Save and Cancel in view.
