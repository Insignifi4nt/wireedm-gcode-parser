...
- Settings expose select, create, duplicate, rename, edit, set-default, and delete operations.
...
2. ensures G40 before any rapid;
3. resolves automatic/manual compensation intent;
4. computes actual winding from final refs;
5. resolves G41/G42 and D selection;
6. validates or generates transitions;
7. emits the operation through the selected dialect;
8. confirms compensation cancellation before the next operation;
9. records traceable blocks and diagnostics for the export preview.
...
- profile selector;
- New, Duplicate, Delete, and Set Default actions;
...
- explicit-linear and Charmilles-native golden post tests using D0;
...
3. Structured compensated post, safe transitions, review UI, and Robofil output. This is the first compensation-ready milestone.
4. Additional controller presets and lead optimization only after the first milestone is verified.
