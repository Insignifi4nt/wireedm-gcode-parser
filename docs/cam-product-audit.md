...
7. Program stop, local `tmp/cam-audit/07-program-stop-marker.png`: a stop with 1 mm remaining appears on the contour with a pause symbol and STOP label. The temporary visual-check edit was discarded and the project returned to Saved. This capture revealed a duplicate-placement error immediately after successful addition; a subsequent fix now acknowledges success while retaining duplicate validation.
...
- Entry/Exit research follow-up: zero-length leads may be accepted and emitted despite being omitted from preview; start/direction edits may preserve review on geometrically changed approaches. Verify appropriate review invalidation and clearance/intersection diagnostics. Fixed lead-out inspection falling through to rapid-in and positioning inspection ignoring the previous exit destination. First positioning inspection now requires a resolved initial wire position. Navigator entry/exit rows now share a compact implementation with strategy, review state and length; selection reveals detailed geometry. Partial/inactive operation travel inspection needs a separate consistency pass.
...
- Stored degenerate leads: execution compilation now blocks entry/exit travel within the preview coincidence tolerance and routes the diagnostic to the affected operation's Entry/Exit tool. Thirteen execution/navigation tests and production build passed, including diagonal coincidence, unchanged source data, and successful compilation after an explicit no-lead correction. This closes the output-boundary gap above; start/direction review invalidation remains pending.
- Start/direction changes: manual leads now require review after reversing cutting direction or relocating a closed start through nearest-point or inferred-point selection. Coordinates remain available for correction; selecting the unchanged start preserves review. Automatic circle-center entry and explicit no-lead decisions retain their semantics. Seventy operation tests plus 90 editor/inference tests passed; production build passed. Clearance/intersection assessment and partial-operation consistency remain pending.
- Integration verification after lead changes: full unit run passed 1,014 tests with one obsolete Statistics `<details>` assertion failing; removed that DOM-shape assertion and all nine tests in its file passed. Full browser run passed 57 scenarios, skipped one optional external-workbench fixture, and failed one obsolete Statistics-summary click; removed that setup step and both diagnostic scenarios passed. Keyboard focus, parsed statistics, import/edit/save/export, magnetic measurement and endpoint hover checks remain intact.
- Active-travel inspection: 111 focused domain/editor tests and production build passed. Inspection derives active operations, skips excluded contours, resolves partial-cut endpoints and their exit leads, and omits travel when participation is unresolved. A 10 mm source cut shortened to 6 mm reports the next rapid from X=6; excluding it reports the rapid from the initial wire position. Canvas selection identity for derived operations still needs an end-to-end pass.
- Partial-cut travel selection: preview positioning/entry/exit paths retain the source operation ID. Eighty-eight preview/editor tests and production build passed. A component workflow clicks a partial exit, verifies its selected canvas state and active 9 mm length, then opens Entry/Exit on the source operation. Derived contour-segment editing/selection and partial exit-review workflow remain separate audit items.
- Partial no-entry review: fixed an unreachable confirmation state where derived no-entry intent required review but the review API accepted only manual leads. Both decisions now use geometry-bound review fingerprints, invalidated by span changes. The participation panel shows review controls only for applicable partial entries and reads effective review instead of a potentially stale stored flag. Ten participation tests, 83 editor tests and production build passed. Partial manual exits still lack a confirmation mechanism and remain a priority follow-up.
- Partial-exit confirmation: added optional persisted exit reviews with geometry fingerprints, validation and portable serialization. Span/geometry changes invalidate confirmation; revocation restores required review. Participation shows the active exit coordinates and confirms/revokes them. Partial transition-review diagnostics open Participation on the source operation. Domain tests cover compile-after-confirm, serialization, revocation and invalidation; component integration covers project save and workflow undo. All 1,021 unit tests across 105 files and production build passed. Rendered/browser inspection of this new control remains pending.
- Partial-exit browser verification: confirmed the active coordinates, review, project save/reload and undo of revocation. This exposed desktop floating panels extending beyond the viewport and making Save unreachable; applying the existing bounds clamp at every viewport width fixed it. The new workflow and all 23 layout browser scenarios pass. Rendered review inspected (`tmp/cam-audit/09-partial-exit-review.png`); the long participation panel still needs content simplification and more prominent actions.
- Participation panel simplification: active ranges are collapsed by default and expand for a selected range. Visible segment labels replace internal IDs, and the exclusion form names its actual action. Rendered 1280 × 720 review now shows Restore, Cancel and Save together without scrolling; refreshed screenshot above. Browser save/reload/undo passes and production build passed. Range units, active-range geometry details and controller-side disclosure still need review.
- Participation validation/scope: blank range fields no longer coerce to zero. The panel and partial review APIs derive the selected source operation independently, so unrelated disconnected cuts cannot prevent local review. A two-operation regression confirms both transitions locally while keeping global execution blocked. Focused domain/panel/editor tests and production build passed.
- Participation geometry details: active ranges show actual derived cutting lengths in millimeters; a split-rectangle regression verifies the ordered lengths 2, 5, 10, 5, 2 mm. Controller-side controls are limited to partial contours and describe explicit source centerline fallback accurately. The panel resolves its partial operation once for range and transition details. Focused panel/editor tests and production build passed.
- Partial machining metrics: corrected cutting totals to include retargeted entry/exit leads and positioning totals to use resolved transition endpoints. A regression verifies 6 mm contour + 8 mm leads = 14 mm cutting, 3 mm initial positioning and 6 mm subsequent positioning. Fifty-nine focused participation/execution/preview/inspection/stop tests and production build passed; remaining-distance stops retain contour-only length semantics.
