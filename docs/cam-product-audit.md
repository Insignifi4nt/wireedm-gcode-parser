...
| Between contours | Explicit defaults, project manual separation and continuous-default display. Routes now follow active operations, including partial cuts and excluded predecessors. Readable summaries, rendered workflow verification and continuous-wire constraints remain | in progress |
...
- Assess lead clearance/intersection diagnostics and review invalidation when local geometry changes without moving its endpoints.
