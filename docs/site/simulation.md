...
Open a path project in the [main app](https://insignifi4nt.github.io/wireedm-gcode-parser/), save it, then choose **Simulation** beside **Editor**. Finish any active editor tool first. Simulation follows the saved UPID's reviewed machining intent; it does not need a generated controller file or an installed postprocessor.
...
Captures are bounded to 1600 pixels per dimension and 1 MiB. Use **Preview PNG** after capture or request a download through the tool. The local image URL is temporary; save the image before leaving the page or replacing the capture. Capture does not save scenario settings or prove which settings were used later. See [Agent tools](agents.md#capture-the-current-preview) for exact arguments, result handling and provenance limits.
